                              ...
                              PointCloudXYZI &pcl_out)
{
    ...
    cout << "[ IMU Process ]: Process lidar from " << pcl_beg_time << " to " << pcl_end_time << ", " << meas.imu.size()
         << " imu msgs from " << imu_beg_time << " to " << imu_end_time << endl;
    ...
    for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
    {
        ...
        // << acc_avr.transpose() << endl; 通过重力数值对加速度进行微调
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
    {
        ...
        // std::cout << "head->offset_time: " << head->offset_time << "     it_pcl->curvature: " << it_pcl->curvature <<
        // std::endl; for(; it_pcl->curvature / double(1000) > head->offset_time; it_pcl --)
        for (; it_pcl->curvature > head->offset_time; it_pcl--)
        {
            ...
            M3D R_i(R_imu * Exp(angvel_avr, dt)); // 当前待补偿点的旋转  Exp(angvel_avr,
                                                  // dt)对角速度积分，通过罗德里格斯公式计算得到运动变换的旋转矩阵

            V3D P_i(it_pcl->x, it_pcl->y, it_pcl->z); // lidar系下点位置
            V3D T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt -
                     imu_state.pos); // 根据IMU数据得到当前点到IMU帧的运动
            V3D P_compensate =
                         ...
                         PointCloudXYZI::Ptr cur_pcl_un_)
{
    ...
    if (imu_need_init_)
    {
      ...
      //          imu_state.grav[0], imu_state.grav[1], imu_state.grav[2], mean_acc.norm(), cov_bias_gyr[0], cov_bias_gyr[1], cov_bias_gyr[2], cov_acc[0], cov_acc[1], cov_acc[2], cov_gyr[0], cov_gyr[1], cov_gyr[2]);
            fout_imu.open(DEBUG_FILE_DIR("imu.txt"), ios::app);
            fout_imu << imu_state.grav[0] << imu_state.grav[1] << imu_state.grav[2] << mean_acc.norm()
                     << cov_bias_gyr[0] << cov_bias_gyr[1] << cov_bias_gyr[2] << cov_acc[0] << cov_acc[1] << cov_acc[2]
                     << cov_gyr[0] << cov_gyr[1] << cov_gyr[2] << std::endl;
            fout_imu.close();
        }
